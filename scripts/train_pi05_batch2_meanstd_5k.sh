...
OUT="outputs/pi05_libero_object_multitask_batch2_meanstd_5k"
RUNLOG="outputs/.runlogs/pi05_libero_object_multitask_batch2_meanstd_5k"
...
/usr/bin/time -v -o "$RUNLOG/training_time.txt" \
lerobot-train \
  ...
  --output_dir="$OUT" \
  --job_name=pi05_libero_object_multitask_batch2_meanstd_5k \
  --batch_size=2 \
  --steps=5000 \
  --seed=42 \
  --num_workers=8 \
...
test -d "$OUT/checkpoints/002500/pretrained_model"
test -d "$OUT/checkpoints/005000/pretrained_model"
