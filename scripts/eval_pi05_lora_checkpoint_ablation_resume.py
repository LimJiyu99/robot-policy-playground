#!/usr/bin/env python3
"""Resumable π0.5 LoRA checkpoint ablation on the established 30-seed protocol."""
...
ROOT = Path("outputs/pi05_lora_checkpoint_ablation_action_steps_10")
RESULT_PATH = ROOT / "instrumentation.json"
DATASET_ROOT = Path("/workspace/jy/datasets/lerobot/libero_object_image")
TRAIN_ROOT = Path("outputs/pi05_libero_object_multitask_lora_meanstd_10k")
CHECKPOINTS = {"5k": "005000", "10k": "010000"}
EPISODES_PER_TASK = 3
...
def load_checkpoint_configs() -> dict[str, dict[str, Any]]:
    ...
    for label, step in CHECKPOINTS.items():
        ...
        if not all(item.is_file() for item in (config_path, adapter_config_path, adapter_weights_path)):
            raise FileNotFoundError(f"Missing π0.5 LoRA checkpoint files for {label}: {path}")

        config = json.loads(config_path.read_text())
        adapter = json.loads(adapter_config_path.read_text())
        if config.get("type") != "pi05" or config.get("chunk_size", 0) < 10:
            raise ValueError(f"Invalid π0.5 config: {config_path}")
        if not config.get("use_peft"):
            raise ValueError(f"Checkpoint is not marked as a PEFT adapter: {config_path}")
        if adapter.get("base_model_name_or_path") != "lerobot/pi05_base":
            raise ValueError(f"Unexpected LoRA base model in {adapter_config_path}")
        if not adapter.get("target_modules") or not adapter.get("modules_to_save"):
            raise ValueError(f"Incomplete LoRA adapter definition: {adapter_config_path}")
        if config.get("normalization_mapping") != {
            "VISUAL": "IDENTITY", "STATE": "MEAN_STD", "ACTION": "MEAN_STD"
        }:
            raise ValueError(f"Expected MEAN_STD mapping in {config_path}")
        configs[label] = {
            "checkpoint": str(path),
            ...
            "lora_target_module_count": len(adapter["target_modules"]),
            "lora_modules_to_save": adapter["modules_to_save"],
...
def make_result(instructions: dict[str, str], configs: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "adapter_loading": "LeRobot use_peft=true loads adapter_config.json on base_model_name_or_path",
        "checkpoints": {
            label: {
...
def refresh_totals(result: dict[str, Any]) -> None:
    ...
    for label, checkpoint in result["checkpoints"].items():
        ...
        if len(episodes) == 30:
            completed.append((label, checkpoint))
    result["best_checkpoint"] = max(completed, key=lambda item: item[1]["overall_success_rate"])[0] if completed else None
...
def validate_existing_result(result: dict[str, Any]) -> None:
    if result.get("policy_type") != "pi05" or result.get("n_action_steps_override") != 10:
        raise ValueError(f"Refusing to mix a different policy protocol into {RESULT_PATH}")
