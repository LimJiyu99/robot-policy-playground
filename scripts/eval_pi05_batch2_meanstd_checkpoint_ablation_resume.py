#!/usr/bin/env python3
"""Resumable π0.5 MEAN_STD checkpoint ablation on the MIN_MAX seed protocol."""
...
ROOT = Path("outputs/pi05_batch2_meanstd_checkpoint_ablation_action_steps_10")
RESULT_PATH = ROOT / "instrumentation.json"
MINMAX_REFERENCE = Path("outputs/pi05_batch2_checkpoint_ablation_action_steps_10/instrumentation.json")
DATASET_ROOT = Path("/workspace/jy/datasets/lerobot/libero_object_image")
TRAIN_ROOT = Path("outputs/pi05_libero_object_multitask_batch2_meanstd_5k")
CHECKPOINTS = {"2.5k": "002500", "5k": "005000"}
EPISODES_PER_TASK = 3
...
def load_minmax_reference() -> dict[str, Any]:
    reference = json.loads(MINMAX_REFERENCE.read_text())
    for label in CHECKPOINTS:
        for task in range(10):
            seeds = [item["seed"] for item in reference["checkpoints"][label]["task_results"][str(task)]["per_episode"]]
            if seeds != expected_seeds(task):
                raise ValueError(f"MIN_MAX reference seeds differ for {label}, task {task}: {seeds}")
    return reference
...
def make_result(instructions: dict[str, str], configs: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "minmax_reference_path": str(MINMAX_REFERENCE),
        "matched_seed_reference_verified": True,
...
def validate_existing_result(result: dict[str, Any]) -> None:
    ...
    if result.get("minmax_reference_path") != str(MINMAX_REFERENCE):
        raise ValueError(f"Refusing to replace the MIN_MAX comparison reference in {RESULT_PATH}")
