#!/usr/bin/env python3
"""Resumable 30-seed evaluation for the new π0.5 expert-only MEAN_STD 40K run."""
...
ROOT = Path("outputs/pi05_expert_only_batch2_meanstd_40k_checkpoint_ablation_action_steps_10")
RESULT_PATH = ROOT / "instrumentation.json"
DATASET_ROOT = Path("/workspace/jy/datasets/lerobot/libero_object_image")
TRAIN_ROOT = Path("outputs/pi05_libero_object_multitask_batch2_meanstd_40k")
CHECKPOINTS = {"10k": "010000", "20k": "020000", "40k": "040000"}
EPISODES_PER_TASK = 3
...
def expected_seeds(task: int) -> list[int]:
    # Matches every prior π0.5 30-episode checkpoint ablation.
...
def load_checkpoint_configs() -> dict[str, dict[str, Any]]:
    ...
    for label, step in CHECKPOINTS.items():
        ...
        if not path.is_dir() or not config_path.is_file() or not weight_path.is_file():
            raise FileNotFoundError(f"Missing complete π0.5 checkpoint for {label}: {path}")
        config = json.loads(config_path.read_text())
        if config.get("type") != "pi05" or config.get("chunk_size") != 50:
            raise ValueError(f"Unexpected π0.5 model config: {config_path}")
        if config.get("n_action_steps") != 10 or config.get("normalization_mapping") != expected_norm:
            raise ValueError(f"Unexpected action-step or normalization config: {config_path}")
        if config.get("use_peft") or not config.get("train_expert_only") or not config.get("freeze_vision_encoder"):
            raise ValueError(f"Expected non-LoRA expert-only configuration: {config_path}")
        configs[label] = {
            "checkpoint": str(path),
...
def main() -> None:
    ...
    result = json.loads(RESULT_PATH.read_text()) if RESULT_PATH.exists() else make_result(instructions, configs)
